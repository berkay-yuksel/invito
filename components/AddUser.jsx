...
const AddUser = ({ users, twitterprofile }) => {
      ...
      switch (e.target.name) {
        case "firstinvite":
          setValidFirstInvite(
            !users.some(
              (person) =>
                person.firstinvite === e.target.value ||
                person.secondinvite === e.target.value ||
                person.account === e.target.value
            )
          );

          break;
        case "secondinvite":
          setValidSecondInvite(
            !users.some(
              (person) =>
                person.firstinvite === e.target.value ||
                person.secondinvite === e.target.value ||
        ...
        case "address":
          setValidAddress(
            !users.some((person) => person.address === e.target.value)
          );
          break;
        default:
      }
  ...
  const handleSubmit = () => {
    if (
      ...
      validSecondInvite &&
      validAddress &&
      user.firstinvite != user.secondinvite
    ) {
      startTransition(() => addUserToDatabase(user));
    } else {
      alert(
        "nah dud! something wrong! probably u just invited the same person"
      );
    }
  };
      ...
      {users.some((person) => person.account === twitterprofile) ? (
        ...
        <form>
          ...
          <input
            ...
            title="The input should start with @ and must be a valid twitter account"
          />
          {validSecondInvite === true
            ? " ✓ "
            : "✖ this account already invited"}
          <br />
          <br />
          ...
          {users.map((item) => (
            <li key={item.id}>
              {item.id}-{item.account}-{item.firstinvite}-2-{item.secondinvite}
              -3-{item.address}{" "}
            </li>
          ))}
        </form>
